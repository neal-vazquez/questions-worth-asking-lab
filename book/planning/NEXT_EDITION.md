...
Extend the existing anthology with Neal's new LinkedIn writing and comments on his own posts, including new follow-ups on posts already represented in the reader. Preserve original prose and the four-part structure.
...
| Canonical manuscript additions | Pending selection | Preserve native backup, add prose, verify structure |
| Public book release | Not performed by this repo update | Record actual distribution separately |

## Acceptance for an incorporation pass

- Every included post and comment has verified authorship and a durable source locator.
- Every included comment is linked to a verified Neal-authored root post; unresolved thread context is held.
- Already-incorporated IDs are reconciled. Edits remain revisions of a source, not duplicate new pieces.
- Preserved text matches the pinned observations exactly; framing and qualifications are separate.
- Capture coverage is stated per root. Newly discovered comments on older posts are included in the review.
- Original prose, manual edits, images, TOC structure, references, acknowledgments, and legal placement survive the manuscript update.
- Release history states the exact additions and remaining gaps. Publication is never inferred from a commit.
